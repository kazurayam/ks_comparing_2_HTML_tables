...
class Record to encapsulate data in a row of HTML table
the Record has getKey() method that makes a combination of CUTOMER+CHANNEL as the key which identifies a row of HTML table uniquely.
use java.util.TreeMap to make a set of Record objects automatically sorted by the key; the Record class implements int compareTo(Record) method.
use java.util.Set that provides removeAll() method which performs substitute operation of mathmatical set; the Record class implements boolean equals(Object) method.

Do you think this is unnecessarily complicated? I don’t think so. This sample code is an absolute minimum to solve the problem that was asked to solve. The problem is a difficult one, it deserves this complexity.
